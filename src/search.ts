...
async function waitForManualVerification(
	...
	if (onVerificationChallenge) {
		try {
			...
			logger.warn(
				{ error, reason },
				'Failed while sending the verification challenge notification.'
			);
		}
	}
...
export async function googleSearch(
	...
	if (fs.existsSync(stateFile)) {
		...
		if (fs.existsSync(fingerprintFile)) {
			try {
				...
				logger.warn(
					{ error: e },
					'Could not load the fingerprint file. Creating a new fingerprint.'
				);
			}
		}
	} else {
	...
	const extractSearchResults = async (page: Page, maxResults: number): Promise<SearchResult[]> => {
	...
	const gotoNextResultsPage = async (page: Page): Promise<boolean> => {
		const nextSelectors = ['#pnnext', 'a[aria-label="Next page"]', 'a[aria-label="Next"]'];

		for (const selector of nextSelectors) {
			const nextLink = await page.$(selector);
			if (!nextLink) continue;

			await Promise.all([page.waitForLoadState('networkidle', { timeout }), nextLink.click()]);
			return true;
		}
				...
				if (!noSaveState) {
					...
					try {
						...
						logger.error(
							...
							'Failed while saving fingerprint configuration.'
						);
					}
				} else {
...
export async function getGoogleSearchPageHtml(
	...
	async function performSearchAndGetHtml(headless: boolean): Promise<HtmlResponse> {
		...
		try {
			...
			// 返回错误信息
			throw new Error(
				`Failed to fetch Google search page HTML: ${error instanceof Error ? error.message : String(error)}`
			);
		}
	}
