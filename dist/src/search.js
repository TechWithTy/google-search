...
async function waitForManualVerification(page, sorryPatterns, timeout, reason) {
...
export async function googleSearch(query, options = {}, existingBrowser) {
    // 设置默认选项
    const { limit = 10, timeout = 60000, stateFile = "./browser-state.json", noSaveState = false, locale = DEFAULT_LOCALE, headless = true, manualVerification = false, googleDomain = DEFAULT_GOOGLE_DOMAIN, reuseBrowserKey, } = options;
...
export async function getGoogleSearchPageHtml(query, options = {}, saveToFile = false, outputPath) {
    ...
    const { timeout = 60000, stateFile = "./browser-state.json", noSaveState = false, locale = DEFAULT_LOCALE, headless = true, manualVerification = false, googleDomain = DEFAULT_GOOGLE_DOMAIN, } = options;
    let useHeadless = manualVerification ? false : headless;
