...
const reusableBrowsers = new Map();
function dropReusableBrowser(reuseBrowserKey) {
    if (reuseBrowserKey) {
...
export async function googleSearch(query, options = {}, existingBrowser) {
    ...
    if (fs.existsSync(stateFile)) {
        logger.info({ stateFile }, "Found a browser state file. Reusing the saved session to reduce anti-bot checks.");
        ...
        logger.info({ stateFile }, "No browser state file found. Creating a new browser session and fingerprint.");
    }
    // 只使用桌面设备列表
    const deviceList = [
        "Desktop Chrome",
    ...
    const normalizeResultLink = (rawLink) => {
        try {
    ...
    const shouldKeepResult = (result) => {
        const title = String(result.title || "").trim();
        const link = String(result.link || "").trim();
        if (!title || !link)
            return false;
        if (/^read more$/i.test(title))
            return false;
        if (/^about this result$/i.test(title))
            return false;
        if (/google\.com\/|accounts\.google|support\.google/i.test(link))
            return false;
        if (/#:~:text=/i.test(String(result.link || "")))
            return false;
        return true;
    };
    const extractSearchResults = async (page, maxResults) => {
        return await page.evaluate((pageLimit) => {
            const results = [];
            ...
            const shouldSkipResult = (title, link) => {
                const normalizedTitle = (title || "").trim();
                const normalizedLink = (link || "").trim();
                if (!normalizedLink || !normalizedLink.startsWith("http"))
                    return true;
                if (/google\.com\/|accounts\.google|support\.google/i.test(normalizedLink))
                    return true;
                if (/#:~:text=/i.test(link))
                    return true;
                if (/^read more$/i.test(normalizedTitle))
                    return true;
                if (/^about this result$/i.test(normalizedTitle))
                    return true;
                return false;
            };
            const selectorSets = [
    ...
    const gotoNextResultsPage = async (page) => {
        const nextSelectors = [
            '#pnnext',
    ...
    async function performSearch(headless) {
        ...
        if (existingBrowser) {
            ...
            logger.info({ headless }, `Preparing to launch the browser in ${headless ? "headless" : "headed"} mode...`);
            // 初始化浏览器，添加更多参数以避免检测
        ...
        if (savedState.fingerprint) {
            ...
            if (hostConfig.deviceName !== deviceName) {
                logger.info({ deviceType: hostConfig.deviceName }, "Using device type based on the host machine settings.");
                // 使用新的设备配置
        ...
        try {
            ...
            if (isBlockedPage) {
                if (headless) {
                    ...
                    if (browserWasProvided) {
                        logger.info("Hit a verification page while using an external browser. Creating a new browser instance...");
            ...
            if (isBlockedAfterSearch) {
                if (headless) {
                    ...
                    // 如果是外部提供的浏览器，不关闭它，而是创建一个新的浏览器实例
                    if (browserWasProvided) {
                        logger.info("Hit a verification page after search while using an external browser. Creating a new browser instance...");
                        // 创建一个新的浏览器实例，不再使用外部提供的实例
                        const newBrowser = await chromium.launch({
                            headless: false, // 使用有头模式
                            timeout: timeout * 2,
            ...
            if (!resultsFound) {
                ...
                if (isBlockedDuringResults) {
                    if (headless) {
                        logger.warn("Detected a verification page while waiting for search results. Restarting the browser in headed mode...");
                        // 关闭当前页面和上下文
                        await page.close();
                        await context.close();
                        // 如果是外部提供的浏览器，不关闭它，而是创建一个新的浏览器实例
            ...
            for (let pageIndex = 0; pageIndex < requestedPages; pageIndex += 1) {
                ...
                for (const rawResult of pageResults) {
                    ...
                    if (!shouldKeepResult(result) || seenLinks.has(result.link))
                        continue;
                ...
                logger.info({ currentPage: pageIndex + 1, requestedPages, collected: aggregatedResults.length }, "Moving to the next Google results page...");
                ...
                results: [
                    {
                        title: "Search failed",
                        link: "",
                        snippet: `Could not complete search. Error: ${error instanceof Error ? error.message : String(error)}`,
                    },
                ],
            };
        }
...
export async function closeReusableGoogleSearchBrowser(reuseBrowserKey) {
